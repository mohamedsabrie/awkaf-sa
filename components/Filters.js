     ...
     <div className="flex items-center   h-full ">
       {branchesResult.map(item => (
         <div className="flex items-center justify-center px-2 bg-gray-100 mx-1" key= {uuidv4()} >
           <XIcon className="h-3 text-gray-800 cursor-pointer hover:scale-110 ml-2 transition ease-in-out duration-200 "  onClick={()=> removeElement(item)} />
           {item}
           </div>
       ))}
     </div>
    ...
    <div id="filters">
     ...
     <div className="relative">
       <span className="absolute flex items-center justify-center right-0 top-0 w-10 h-[28px] bg-white border"  onClick={handleShowBranches}>
        {showBranches ? <ChevronUpIcon className="h-4" />: <ChevronDownIcon className="h-4" /> } 
       </span>
       <div
         ref={inputRef}
       ...
       <Transition
         ...
         show={showBranches}
       >
         <div className="absolute  z-20 flex flex-col top-full mt-1 left-0 w-full bg-white py-2 px-6">
